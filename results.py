...
def main():
    ...
    means = grouped['results.best.solution.performance'].agg(trimmed_mean)
    errors = grouped['results.best.solution.performance'].agg(trimmed_std)

    plt.errorbar(grouped.groups.keys(), means, yerr=errors, fmt='-o', capsize=6)
