...
class AlgorithmConfig:
    def __init__(self, population_size, class_name, selection_config_a: SelectionConfig, selection_config_b: SelectionConfig, selection_config_rate,
...
def algorithm(population_to_keep, hard_cap, config: AlgorithmConfig):
    ...
    # algorithm_data = AlgorithmData(population, (current_best, 0))
    # best = current_best, algorithm_data.generation
    # while not should_end(algorithm_data.generation, algorithm_data.population, algorithm_data.best[0], config.end_condition_config) and algorithm_data.generation < hard_cap:
    #     algorithm_data.generation += 1
    #     algorithm_data.population = algorithm_iteration(algorithm_data, population_to_keep, config)
    #     current_best = max(algorithm_data.population, key=lambda x: x.performance)
    #     algorithm_data.last_bests.append((current_best, algorithm_data.generation))
    #     if current_best.performance > algorithm_data.best[0].performance:
    #         algorithm_data.best = current_best, algorithm_data.generation
    #         best = algorithm_data.best
    #     # with open("log.txt", "a") as file:
    #     #     file.write(f"Iteration: {algorithm_data.generation}, best: {current_best}, BEST: {current_best.performance:.2f}, AVG: {np.mean([x.performance for x in population]):.2f}, HSTD: {np.std([x.variables.height for x in population])} \n")  
...
def main():
    ...
    algorithm_config = AlgorithmConfig(population_size, config['algorithm_config']["class_name"], selection_config_a, selection_config_b, config["selection_config"]["ratio_A"],
